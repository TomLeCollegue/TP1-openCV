...
Mat tramage_floyd_steinberg(Mat input, vector<Vec3f> colors)
{
    ...
    for (int line = 1; line < calcul.rows - 1; line++)
    {
        for (int column = 1; column < calcul.cols - 1; column++)
        {
            ...
            calcul.at<Vec3f>(line, column + 1) = applyError(calcul.at<Vec3f>(line, column + 1), errorColor, 7.0f / 16.0f);
            calcul.at<Vec3f>(line + 1, column - 1) = applyError(calcul.at<Vec3f>(line + 1, column - 1), errorColor,3.0f / 16.0f );
            calcul.at<Vec3f>(line + 1, column) = applyError(calcul.at<Vec3f>(line + 1, column), errorColor, 5.0f / 16.0f);
            calcul.at<Vec3f>(line + 1, column + 1) = applyError(calcul.at<Vec3f>(line + 1, column + 1) , errorColor, 1.0f / 16.0f);
        }
    }
